...
<?php
	...
	$sql = "SELECT * FROM bususe WHERE busno = '$busno' AND drvid = '$drvid' AND dte = '$dte' AND timestr = '$timestr'"; //คำสั่ง select ข้อมูลผู้ป่วยจากตาราง patients ที่มี ptid = $ptid
...
<table>
<tr><th><center><h3>Edit Bus Use Form</h3></center><th></tr>
...
<tr><td>dte: </td><td><input type="date" name="dte" value="<?php echo $row["dte"] ?>" disabled></td></tr>
<tr><td>timestr: </td><td><input type="time" name="timestr" value="<?php echo $row["timestr"] ?>" disabled></td></tr>
<tr><td>timestp: </td><td><input type="time" name="timestp" value="<?php echo $row["timestp"] ?>"></td></tr>
...
<?php 
	...
	echo "<input type='hidden' name ='timestr'  value = '$timestr'/>";
?>

<tr><th colspan="2"><center><input type="submit" class='btn btn-success' name="updateBusUse" value="บันทึกการแก้ไข"></center></th></tr>
</table>
</form>
<br>
</center>
