...
//count all patients from database order by name, this is used for caculate the numbers of pages
	$sql = "SELECT count(*) busno FROM dayuse";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		echo "<td>";
		echo "<form action = 'editDayUseForm.php' method ='post'> ";
		...
		echo "<input name = 'editDayuse' type='submit' class='btn btn-warning' value='edit' />";
		echo "</form>";
		echo "</td>";
		echo "<td>";
		...
		echo "<input type='hidden' name ='drvid'  value = '".$row["drvid"]."'/>";
		echo "<input type='hidden' name ='dte'  value = '".$row["dte"]."'/>";	
		echo "<input name = 'delDayuse' type='submit' class='btn btn-danger' value='del' />";
		echo "</form>";
		echo "</td>";
		echo "</tr>";	
    }
...
echo "<tr><td colspan='13'><center>";
	echo"<form action = 'showDayUse.php' method ='post'> ";
