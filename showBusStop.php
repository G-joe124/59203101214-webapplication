...
//count all patients from database order by name, this is used for caculate the numbers of pages
	$sql = "SELECT count(*) bus_stopno FROM bus_stop";
	$result = $conn->query($sql);
	$rw = $result->fetch_assoc(); 
	$numfound = $rw['bus_stopno']; //return the number of records
...
$sql = "SELECT * 
			FROM bus_stop 
			ORDER BY bus_stopno LIMIT $start , $p_size";
			
$result = $conn->query($sql);

echo"<center>";	
echo "<h2>All Bus Stop</h2>";
echo "<table>";
echo "<tr>";
echo "<th><center>ลำดับที่</center></th>";
echo "<th>Bus Stop NO.</th>";
echo "<th>Bus Stop Name</th>";
echo "<th>Latitude</th>";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		echo "<td>";
		echo "<form action = 'editBusStopForm.php' method ='post'> ";
		echo "<input type='hidden' name ='bus_stopno'  value = '".$row["bus_stopno"]."'/>";		
		echo "<input name = 'editBusStop' type='submit' class='btn btn-warning' value='edit' />";
		echo "</form>";
		echo "</td>";
		echo "<td>";
		echo "<form action = 'operations.php' method ='post'> ";
		echo "<input type='hidden' name ='bus_stopno'  value = '".$row["bus_stopno"]."'/>";
		echo "<input name = 'delBusStop' type='submit' class='btn btn-danger' value='del' />";
		echo "</form>";
		echo "</td>";
		echo "</tr>";	
    }
...
echo "<tr><td colspan='13'><center>";
	echo"<form action = 'showBusStop.php' method ='post'> ";
	echo "<input type='hidden' name ='bus_stopno'  value = '$bus_stopno'/>";		
	echo "<input type='hidden' name ='bus_stopnme'  value = '$bus_stopnme'/>";		
...
echo "<tr><td colspan='11'><a href='insertBusstop.php'>Add New Bus Stop</a></td></tr>";
echo"</center>";	
$conn->close();
?>
</body>
