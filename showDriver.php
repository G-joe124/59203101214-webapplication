...
//count all patients from database order by name, this is used for caculate the numbers of pages
	$sql = "SELECT count(*) drvid FROM driver";
	$result = $conn->query($sql);
	$rw = $result->fetch_assoc(); 
	$numfound = $rw['drvid']; //return the number of records
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td>".$row["tel"]."</td>";
        echo "<td>".$row["licenNo"]."</td>";
        echo "<td>".$row["email"]."</td>";
        echo "<td>".$row["regisdte"]."</td>";
		echo "<td>";
		echo "<form action = 'editDriverForm.php' method ='post'> ";
		echo "<input type='hidden' name ='drvid'  value = '".$row["drvid"]."'/>";		
		echo "<input name = 'editPatient' class='btn btn-warning' type='submit' value='edit' />";
		echo "</form>";
		echo "</td>";
		echo "<td>";
		echo "<form action = 'operations.php' method ='post'> ";
		echo "<input type='hidden' name ='drvid'  value = '".$row["drvid"]."'/>";
		echo "<input name = 'delDriver' class='btn btn-danger' type='submit' value='del' />";
		echo "</form>";
		echo "</td>";
		echo "</tr>";	
    }
...
echo "<tr><td colspan='13'><center>";
	echo"<form action = 'showDriver.php' method ='post'> ";
	echo "<input type='hidden' name ='drvid'  value = '$drvid'/>";		
	echo "<input type='hidden' name ='fnme'  value = '$fnme'/>";		
	echo "<input type='hidden' name ='lnme' value = '$lnme'/>";
	echo "<input type='hidden' name ='tel' value = '$tel'/>";
    echo "<input type='hidden' name ='licenNo'  value = '$licenNo'/>";
...
echo "<tr><td colspan='11'><a href='insertDriver.php'>Add New Driver</a></td></tr>";
echo"</center>";	
$conn->close();
?>
</body>
