...
<html>
<head>
<title>Show Bus Driver Schedule</title>
  <meta charset="utf-8">
...
$statuss= array();
$statuss[1]="ACTIVE"; $statuss[2]="UNACTIVE";
...
//count all patients from database order by name, this is used for caculate the numbers of pages
	$sql = "SELECT count(*) dte FROM busdrvsschedule";
	$result = $conn->query($sql);
	$rw = $result->fetch_assoc(); 
	$numfound = $rw['dte']; //return the number of records
...
$sql = "SELECT * 
			FROM busdrvsschedule 
			ORDER BY dte LIMIT $start , $p_size";
			
$result = $conn->query($sql);
echo"<center>";	
echo "<h2>All Bus Driver Schedule</h2>";
echo "<table>";
echo "<tr>";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		echo "<td>".$row["khatime"]."</td>";
        echo "<td>".$row["drvid"]."</td>";
		// echo "<td>".$row["statuss"]."</td>";
		echo "<td>".$statuss[$row["statuss"]]."</td>";
        // echo "<td>".$row["email"]."</td>";
        // echo "<td>".$row["regisdte"]."</td>";
		echo "<td>";
		echo "<form action = 'editBusdrvsscheduleForm.php' method ='post'> ";
		echo "<input type='hidden' name ='dte'  value = '".$row["dte"]."'/>";		
		echo "<input type='hidden' name ='busno'  value = '".$row["busno"]."'/>";	
		echo "<input type='hidden' name ='kha'  value = '".$row["kha"]."'/>";	
		echo "<input name = 'editBusdrvsscheduleForm' type='submit' class='btn btn-warning' value='edit' />";
		echo "</form>";
		echo "</td>";
		echo "<td>";
		...
		echo "<input type='hidden' name ='busno'  value = '".$row["busno"]."'/>";
		echo "<input type='hidden' name ='kha'  value = '".$row["kha"]."'/>";
		echo "<input name = 'delbusdrvsschedule' type='submit' class='btn btn-danger' value='del' />";
		echo "</form>";
		echo "</td>";
		echo "</tr>";	
    }
...
echo "<tr><td colspan='13'><center>";
	echo"<form action = 'showBusdrvsschedule.php' method ='post'> ";
	...
	echo "<input type='hidden' name ='khatime' value = '$khatime'/>";
...
echo "<tr><td colspan='11'><a href='insertBusdrvsschedule.php'>Add New Driver Schedule</a></td></tr>";
echo"</center>";
$conn->close();
?>
</body>
