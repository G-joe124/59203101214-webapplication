...
//count all patients from database order by name, this is used for caculate the numbers of pages
	$sql = "SELECT count(*) busno FROM bususe";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		echo "<td>";
		echo "<form action = 'editBusUseForm.php' method ='post'> ";
		...
		echo "<input type='hidden' name ='timestr'  value = '".$row["timestr"]."'/>";		
		echo "<input name = 'editBusUse' type='submit' class='btn btn-warning' value='edit' />";
		echo "</form>";
		echo "</td>";
		echo "<td>";
		...
		echo "<input name = 'delBusUse' type='submit' class='btn btn-danger' value='del' />";
		echo "</form>";
		echo "</td>";
		echo "</tr>";	
    }
...
echo "<tr><td colspan='13'><center>";
	echo"<form action = 'showBusUse.php' method ='post'> ";
...
echo "<tr><td colspan='11'><a href='insertBusstop.php'>Add New Bus</a></td></tr>";
echo"</center>";	
$conn->close();
?>
</body>
